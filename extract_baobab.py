...
Destination SharePoint :
    SHAREPOINT_FOLDER_LINK_BAOBAB

Usage :
    python extract_baobab.py
"""
...
def build_summary_text(stats: dict) -> str:
    lines = [f"Extraction BAOBAB terminee le {date.today().isoformat()}."]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Extraction BAOBAB : pieces jointes des emails envoyes par BAOBAB, dans les boites donnees.")
    parser.add_argument("--senders", nargs="+", default=DEFAULT_SENDERS)
    parser.add_argument("--output-dir", default="./baobab")
    parser.add_argument("--sharepoint-link", default=os.environ.get("SHAREPOINT_FOLDER_LINK_BAOBAB"))
        ...
        if email_enabled:
            try:
                send_email(session, args.email_sender, args.email_recipients,
                           f"[ECHEC] Extraction BAOBAB - {date.today().isoformat()}",
                           f"L'extraction BAOBAB a echoue.\n\nErreur :\n{error_text}")
    ...
    if email_enabled:
        try:
            send_email(session, args.email_sender, args.email_recipients,
                       f"Extraction BAOBAB - {date.today().isoformat()} : {stats['total_files']} fichier(s)",
